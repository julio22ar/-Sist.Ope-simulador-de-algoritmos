...
static int accederCacheReloj(cacheReloj *cache, int pagina, int *victima) {
...
for (int i = 0; i < cantidad; i++) {
    ...
    int victimaLRU = -1, victimaReloj = -1;
    int missLRU = accederCacheLRU(lru, pagina, &victimaLRU);
    int missReloj = accederCacheReloj(reloj, pagina, &victimaReloj);

    if (missLRU) fallosLRU++;
    if (missReloj) fallosReloj++;

    printf("\nReferencia: %d\n", pagina);
    printf("  LRU -> %s", missLRU ? "MISS" : "HIT ");
    if (victimaLRU != -1) printf(" | Reemplazó página %d", victimaLRU);
    printf("\n");

    printf("  Reloj -> %s", missReloj ? "MISS" : "HIT ");
    if (victimaReloj != -1) printf(" | Reemplazó página %d", victimaReloj);
    printf("\n");
}
